// ==================== 内置词典（动态加载） ====================
// 注意：词典数据已移至 public/dict_builtin.json 和 public/dict_builtin_en.json
// 使用 loadBuiltinDictionary() 和 loadBuiltinDictionary() 异步加载

export type BuiltinDictionary = Record<string, { meaning: string; pos: string }>;
export type BuiltinDictionaryEn = Record<string, string>;

// 模块级词典缓存
let builtinDictionary: BuiltinDictionary | null = null;
let builtinDictionaryEn: BuiltinDictionaryEn | null = null;

/**
 * 加载内置英汉词典（异步）
 */
export async function loadBuiltinDictionary(): Promise<void> {
  if (builtinDictionary) return; // Already loaded
  
  try {
    const response = await fetch('/dict_builtin.json');
    if (response.ok) {
      builtinDictionary = await response.json();
      console.log('内置词典已加载:', Object.keys(builtinDictionary || {}).length, '词条');
    }
  } catch (error) {
    console.warn('加载内置词典失败:', error);
  }
}
...
export async function loadBuiltinDictionaryEn(): Promise<void> {
  if (builtinDictionaryEn) return; // Already loaded
  
  try {
    const response = await fetch('/dict_builtin_en.json');
    if (response.ok) {
      builtinDictionaryEn = await response.json();
      console.log('内置英英词典已加载:', Object.keys(builtinDictionaryEn || {}).length, '词条');
    }
  } catch (error) {
    console.warn('加载内置英英词典失败:', error);
  }
}
 ...
 * 获取内置词典是否已加载
 */
export function isBuiltinDictionaryLoaded(): boolean {
  return builtinDictionary !== null;
}

/**
 * 获取内置英英词典是否已加载
 */
export function isBuiltinDictionaryEnLoaded(): boolean {
  return builtinDictionaryEn !== null;
}
...
export function smartLookup(word: string): { meaning: string; pos: string } | null {
  ...
  // 1. 先查原始单词
  if (builtinDictionary[lower]) {
    return builtinDictionary[lower];
  }
  ...
  for (const variant of variants) {
    if (builtinDictionary[variant]) {
      return builtinDictionary[variant];
    }
  }
  
  // 4. 最后尝试lemmatize结果
  const lemma = lemmatize(lower);
  if (lemma !== lower && builtinDictionary[lemma]) {
    return builtinDictionary[lemma];
  }
...
// ==================== 英英词典查找（动态） ====================
// 注意：英英词典数据已移至 public/dict_builtin_en.json
// 使用 loadBuiltinDictionaryEn() 异步加载
...
export function getWordMeaningEn(word: string): string | null {
  ...
  // 1. 直接查找
  if (builtinDictionaryEn[lower]) {
    return builtinDictionaryEn[lower];
  }
  
  // 2. 获取词根变体
  const variants = getStemVariants(lower);
  for (const variant of variants) {
    if (builtinDictionaryEn[variant]) {
      return builtinDictionaryEn[variant];
    }
  }
  
  // 3. 最后尝试 lemmatize 结果
  const lemma = lemmatize(lower);
  if (lemma !== lower && builtinDictionaryEn[lemma]) {
    return builtinDictionaryEn[lemma];
  }
